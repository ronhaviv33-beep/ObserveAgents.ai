...
// ─── Organizations management page (platform admin only) ─────────────────────
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16, maxWidth:900 }}>
      ...
      <div style={{ background:T.panel, border:`1px solid ${T.border}`, borderRadius:8, padding:"20px 24px" }}>
        ...
        <div style={{ fontSize:11, color:T.textMute, marginBottom:16, fontFamily:FONT_MONO_S }}>
          A default admin user and roles (admin / analyst / viewer) will be created automatically.
        </div>
        <form onSubmit={handleCreate} style={{ display:"flex", gap:12, flexWrap:"wrap", alignItems:"flex-end" }}>
          <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
            <label style={{ fontSize:9, fontFamily:FONT_MONO_S, letterSpacing:"0.12em", textTransform:"uppercase", color:T.textMute }}>Organization Name *</label>
            <input
              value={form.name}
              onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
              placeholder="e.g. Acme Corp"
              required
              style={{ background:T.panelHi, color:T.text, border:`1px solid ${T.border}`, padding:"7px 12px", borderRadius:4, fontSize:13, fontFamily:FONT_MONO_S, width:200 }}
            />
          </div>
          <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
            <label style={{ fontSize:9, fontFamily:FONT_MONO_S, letterSpacing:"0.12em", textTransform:"uppercase", color:T.textMute }}>Admin Email *</label>
            <input
              type="email"
              ...
              required
              style={{ background:T.panelHi, color:T.text, border:`1px solid ${T.border}`, padding:"7px 12px", borderRadius:4, fontSize:13, fontFamily:FONT_MONO_S, width:220 }}
            />
          </div>
          <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
            <label style={{ fontSize:9, fontFamily:FONT_MONO_S, letterSpacing:"0.12em", textTransform:"uppercase", color:T.textMute }}>Admin Name</label>
            <input
              value={form.admin_name}
              onChange={e => setForm(f => ({ ...f, admin_name: e.target.value }))}
              placeholder="Admin"
              style={{ background:T.panelHi, color:T.text, border:`1px solid ${T.border}`, padding:"7px 12px", borderRadius:4, fontSize:13, fontFamily:FONT_MONO_S, width:160 }}
            />
          </div>
          <button type="submit" disabled={saving || !form.name.trim() || !form.admin_email.trim()}
            style={{ background:T.accent, color:T.bg, border:"none", padding:"8px 20px", borderRadius:4, fontSize:12, fontFamily:FONT_MONO_S, fontWeight:600, cursor:"pointer", opacity:(saving||!form.name.trim()||!form.admin_email.trim())?0.5:1, whiteSpace:"nowrap", height:34, alignSelf:"flex-end" }}>
            {saving ? "Creating…" : "+ Create Organization"}
          </button>
        </form>

        {err && (
          <div style={{ marginTop:12, color:T.crit, fontFamily:FONT_MONO_S, fontSize:12, background:`${T.crit}10`, border:`1px solid ${T.crit}33`, borderRadius:4, padding:"8px 12px" }}>{err}</div>
        )}
        ...
        {success && created && (
          <div style={{ marginTop:12, background:`${T.accent}10`, border:`1px solid ${T.accent}44`, borderRadius:6, padding:"14px 16px", display:"flex", flexDirection:"column", gap:8 }}>
            ...
            <div style={{ fontSize:11, color:T.textDim, fontFamily:FONT_MONO_S }}>Default roles (admin / analyst / viewer) were created for this organization.</div>
            {created.admin_temporary_password && (
              <div style={{ background:T.bg, border:`1px solid ${T.warn}44`, borderRadius:4, padding:"10px 14px" }}>
                <div style={{ fontSize:9, fontFamily:FONT_MONO_S, color:T.warn, letterSpacing:"0.12em", textTransform:"uppercase", marginBottom:6 }}>Temporary Admin Password — copy now, not shown again</div>
                <div style={{ fontFamily:FONT_MONO_S, fontSize:13, color:T.text, letterSpacing:"0.05em" }}>{created.admin_temporary_password}</div>
                <div style={{ fontSize:10, color:T.textMute, fontFamily:FONT_MONO_S, marginTop:6 }}>
      ...
      <div style={{ background:T.panel, border:`1px solid ${T.border}`, borderRadius:8, overflow:"hidden" }}>
        <div style={{ padding:"16px 20px", borderBottom:`1px solid ${T.border}`, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div style={{ fontSize:13, fontWeight:600, color:T.text }}>
            All Organizations
            {orgs.length > 0 && <span style={{ fontFamily:FONT_MONO_S, fontSize:11, color:T.textMute, marginLeft:8 }}>· {orgs.length}</span>}
          </div>
          <button onClick={load} style={{ background:"transparent", border:`1px solid ${T.border}`, color:T.textDim, padding:"4px 12px", borderRadius:4, fontSize:11, fontFamily:FONT_MONO_S, cursor:"pointer" }}>
            Refresh
          </button>
        </div>

        {loading ? (
          <div style={{ color:T.textMute, fontFamily:FONT_MONO_S, fontSize:12, padding:"24px 20px" }}>Loading organizations…</div>
        ) : orgs.length === 0 ? (
          <div style={{ padding:"32px 20px", textAlign:"center" }}>
            ...
            <div style={{ fontSize:12, color:T.textMute, fontFamily:FONT_MONO_S }}>Create your first customer organization to start onboarding AI assets.</div>
          </div>
        ) : (
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            <thead>
              <tr style={{ borderBottom:`1px solid ${T.border}` }}>
                {["ID","Name","Slug","Created","Users",""].map(h => (
                  <th key={h} style={{ textAlign:"left", padding:"10px 16px", fontFamily:FONT_MONO_S, fontSize:10, letterSpacing:"0.1em", textTransform:"uppercase", color:T.textMute, fontWeight:500, background:T.panelHi }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {orgs.map(o => (
                <tr key={o.id} style={{ borderBottom:`1px solid ${T.border}` }}>
                  <td style={{ padding:"14px 16px", fontFamily:FONT_MONO_S, fontSize:11, color:T.textMute }}>{o.id}</td>
                  <td style={{ padding:"14px 16px", fontSize:13, color:T.text, fontWeight:500 }}>
                    {o.name}
                    {o.is_internal && (
                      <span style={{ marginLeft:8, fontSize:9, fontFamily:FONT_MONO_S, color:purple, background:`${purple}18`, border:`1px solid ${purple}44`, borderRadius:3, padding:"1px 6px", textTransform:"uppercase", letterSpacing:"0.08em" }}>platform</span>
                    )}
                  </td>
...
function SimpleIntegrationsPage({ onNavigate }) {
  ...
  const [section, setSection] = useState(null); // "gateway" | "sdk" | "platform" | null
  ...
  // ── Setup options (Route Traffic first — recommended) ─────────────────────
  const OPTIONS = [
    {
      id:    "gateway",
      badge: "Recommended",
    ...
    },
    {
      ...
      benefits: [
        "Add rich identity metadata",
        "Improve team and owner attribution",
        "Track environment and version",
        "Increase discovery confidence",
      ],
    ...
    },
    {
      ...
      benefits: [
        "Discover GitHub, Slack, Jira, ServiceNow, and MCP signals",
        "Find potential AI assets",
  ...
  // ── Technical data (shown only after clicking a CTA) ──────────────────────
  const GW_FLOW = [
...
curl GATEWAY_URL/v1/chat/completions \\
  ...
  return (
    <div style={{ maxWidth:900, margin:"0 auto", padding:"32px 24px", fontFamily:FONT_UI }}>
      ...
      {/* ── Technical instructions — shown after clicking a CTA ── */}
      ...
      {section === "gateway" && (
        <div style={{ border:`1px solid ${T.info}44`, borderRadius:10, padding:"24px 28px", marginBottom:16 }}>
          ...
          <div style={{ fontSize:12, color:T.textDim, lineHeight:1.7, marginBottom:20 }}>
            ...
            The gateway analyses every request and derives agent identity from available signals.
          ...
          <div style={{ background:`${T.info}0d`, border:`1px solid ${T.info}33`, borderRadius:8, padding:"12px 16px" }}>
            <div style={{ fontSize:12, color:T.textDim, lineHeight:1.65 }}>
              ...
              Optionally add <code style={{ fontFamily:FONT_MONO, color:T.textMute, fontSize:11 }}>X-Agent-*</code> headers or the SDK for higher-confidence attribution.
            </div>
          </div>
        </div>
      )}
      ...
      {section === "sdk" && (
        <div style={{ border:`1px solid ${"#34d399"}44`, borderRadius:10, padding:"24px 28px", marginBottom:16 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:24, marginBottom:20 }}>
            <div>
              <div style={{ fontSize:12, color:T.textDim, lineHeight:1.7, marginBottom:16 }}>
                The SDK automatically collects runtime context — service name, environment, version, and framework metadata —
                and attaches it to every request. This improves discovery accuracy and reduces manual configuration.
              </div>
            ...
            <div>
              ...
              {[
                ...
                ["Hostname",                 "Stored as metadata only"],
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:8, marginBottom:20 }}>
            ...
            <CodeBlock id="sdk_env"       label="Env-var · httpx · LangChain"     snippet={resolvedSnippets.sdk_env}       accentColor="#34d399" />
          </div>
          ...
          <div style={{ background:`${T.warn}08`, border:`1px solid ${T.warn}22`, borderRadius:8, padding:"14px 18px" }}>
            ...
            <div style={{ fontSize:12, color:T.textDim, marginBottom:12, lineHeight:1.65 }}>
              ...
              All X-Agent-* and X-Guard-* headers remain supported.
            </div>
            ...
            <div>
              ...
              <div style={{ marginTop:16, background:`${T.purple}0d`, border:`1px solid ${T.purple}33`, borderRadius:8, padding:"10px 14px" }}>
                ...
                <div style={{ fontSize:12, color:T.textDim, lineHeight:1.6 }}>
                  <strong style={{ color:T.text }}>Verified</strong> — came through the gateway<br/>
                  <strong style={{ color:T.text }}>Potential</strong> — discovered from platform scan, no gateway traffic yet
                </div>
              </div>
            </div>
            <div>
              ...
              <div style={{ fontSize:12, color:T.textDim, lineHeight:1.65 }}>
                Platform integrations scan for AI API usage patterns, environment variables referencing AI providers,
                workflow nodes, and SDK dependencies — without requiring agent code changes.
              </div>
            </div>
          </div>

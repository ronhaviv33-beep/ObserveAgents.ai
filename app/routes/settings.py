...
def set_org_config_endpoint(key: str, body: dict = Body(...), user=Depends(require_admin), db: Session = Depends(get_db)):
    ...
    if not org:
        raise HTTPException(status_code=404, detail="Organisation not found")
    if key == ORG_CONFIG_KEY:
        errors = validate_attribute_mapping(body.get("value"))
        if errors:
            raise HTTPException(status_code=400, detail={
...
@router.get("/settings/otel-attribute-mapping", tags=["Auth — Users"])
def get_otel_attribute_mapping(user=Depends(get_current_user), db: Session = Depends(get_db)):
    """The org's custom→canonical OTel attribute mapping, plus the allowlist
    of canonical targets (for the future mapping UI's dropdown)."""
    from app.otel_attribute_mapping import ALLOWED_TARGETS, MAX_ENTRIES, ORG_CONFIG_KEY
    mapping = _get_org_config(db, user.organization_id, ORG_CONFIG_KEY)
    return {

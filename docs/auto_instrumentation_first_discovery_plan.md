...
|---|---|---|---|
| `app/risk_processor.py` ~207–217 | `missing_owner` +10, `missing_team` +10, `unknown_environment` +10–15 risk score, was default-on | Was: ~30–45 baseline risk on metadata-free events | **✅ Resolved** — the three governance rules ship disabled by default (`_DEFAULT_OFF_RULES`); a metadata-free clean event scores 0/allow. Rules stay in the catalog and per-org `DetectionRule` overrides re-enable them for orgs that want metadata discipline |
| `app/telemetry_ingest/normalizer.py` | Batch-ingest API hard-required `agent_id` and hardcoded `identity_tier="declared"` | Was: the platform's only ingestion door that rejected evidence on missing identity | **✅ Resolved** — `agent_id` optional; tiered ladder agent_id → agent_name → service attr → privacy-safe stable fingerprint, honest tier passed to the shared `_upsert_asset` (fallback ⇒ confidence 30 + `needs_admin_review`); existing agent_id callers byte-identical |
